...
public class discussion extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		ForumID = request.getParameter("ForumPostID");//if forum id is null get the id from the session
		if(ForumID == null) {
			ForumID = (String) request.getSession().getAttribute("postID");
		}
		...
		try {
			...
			if(dbms.getWhetherCanCommentCommentModel(username, ddd)) {
				out.println(
						"					<div id='firstly'>"
			...
			for(CommentModel d:cc) {
				...
				if(d.getPostID() == ddd) {
					...
					dumm = dbms.getUserProfile(d.getiGN());

	
					if (dumm.getProfilePic() != 0) {
						out.print("<img src='Image/" + dbms.getImageByImageID(dumm.getProfilePic()) + "' height='60' width='60'/>");
					}
					else {
						out.print("<img src='images/profile.png' height='60' width='60'/>");
					}
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(download == null || download.isEmpty()) {
			...
			String postid = request.getParameter("jesus");
